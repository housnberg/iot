...
app.get('/orientation', function(req, res) {
    if (coordinates !== undefined && coordinates.ax < 0.1  && coordinates.ay < 0.1 && coordinates.az < 0.8) {
        res.text("The smartphone is not tilt.");
    } else {
        res.text("The smartphone is tilt.");
    }
});
...
appClient.on("connect", function () {
    ...
    appClient.subscribeToDeviceEvents("iot-phone","eugen","+","json");
    appClient.subscribeToDeviceEvents("iot-phone","patrigg","+","json");
